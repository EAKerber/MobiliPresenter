...
from tools import continuation_transition as transition, maintenance_live
...
def main():
    ...
    current=a.observe().items[task];p=transition.done(current);done=a.apply(p,p["planHash"]);final=maintenance_live.inspect()
