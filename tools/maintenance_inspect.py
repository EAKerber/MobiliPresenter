...
ERROR_EXIT=2
SCHEMA_VERSION="MaintenanceInspection 0.3"
...
def _work_candidate(work_items,graph):
    ...
    if handoffs:
        work_id=handoffs[0];item=_work_item(work_items,work_id);target=item.get("handoffToWorkerId")
        if not isinstance(target,str) or not target:raise RuntimeError("MAINTENANCE_HANDOFF_TARGET_INVALID")
    ...
    if runnable:
        work_id=runnable[0];item=_work_item(work_items,work_id)
...
def _pending_work_pause(work_items,graph):
    ...
    for item in work_items:
        ...
        work_id=str(item.get("id") or "");status=item.get("status")
        if status=="WAITING":candidates.append((work_id,"WORK_WAITING","; ".join(str(v) for v in item.get("blockers") or [])))
    for work_id in graph.get("dependencyBlocked") or []:
        item=_work_item(work_items,work_id);pending=[dep for dep in item.get("dependsOn") or [] if dep not in set(graph.get("terminal") or [])];candidates.append((str(work_id),"WORK_DEPENDENCY_BLOCKED",f"waiting for dependencies: {', '.join(sorted(str(v) for v in pending))}"))
...
def decide(state,verification,capabilities,*,remote_requested,pull_requests,coordination_state,work_items=None,work_graph=None,machine_trust=None,machine_coherence=None,machine_sensors=None):
    findings=_machine_findings(machine_trust,machine_coherence,machine_sensors);work_items=work_items or [];graph=work_graph if isinstance(work_graph,dict) else work_graph_module.build(work_items);work_graph_module.validate(graph)
    ...
    if work_candidate is not None:findings.append(work_candidate)
    ...
    if work_candidate is None and not any(item.get("action") in {"CONTINUE","HANDOFF"} for item in findings):
        ...
        if pending is not None:findings.append(pending)
...
def build_inspection(state,verification,observed_git,capabilities,*,remote_requested,pull_requests,coordination_state,work_items=None,work_graph=None,machine_trust=None,machine_coherence=None,machine_sensors=None):
    work_items=work_items or [];graph=work_graph if isinstance(work_graph,dict) else work_graph_module.build(work_items);findings,recommendation=decide(state,verification,capabilities,remote_requested=remote_requested,pull_requests=pull_requests,coordination_state=coordination_state,work_items=work_items,work_graph=graph,machine_trust=machine_trust,machine_coherence=machine_coherence,machine_sensors=machine_sensors);body={"schemaVersion":SCHEMA_VERSION,"repository":state["project"]["repository"],"projectState":{"phase":state["development"]["phase"],"checkpoint":state["development"]["checkpoint"],"nextTransition":state["development"]["nextTransition"],"activeDevelopmentBranch":state["git"].get("activeDevelopmentBranch"),"developmentPrNumber":state["development"].get("prNumber"),"blockers":state["development"].get("blockers") or []},"verification":verification,"observedGit":observed_git,"capabilities":capabilities,"workItems":work_items,"workGraph":graph,"remoteRequested":remote_requested,"pullRequests":pull_requests,"coordination":coordination_state,"findings":findings,"recommendation":recommendation,"readOnly":True};return {**body,"inspectionHash":stable_hash(body)}
...
def from_project_inspection(machine):
    ...
    if not isinstance(work_items,list):raise RuntimeError("PROJECT_MACHINE_WORK_ITEMS_INVALID")
...
def main(argv=None):
    ...
    try:
        payload=inspect(args.remote);print(json.dumps(payload,indent=2,ensure_ascii=False) if args.as_json else "MAINTENANCE INSPECT\n  recommendation: %s\n  reason: %s\n  focus: %s\n  work items: %d\n  inspectionHash: %s"%(payload["recommendation"]["action"],payload["recommendation"]["reasonCode"],payload["recommendation"].get("focus") or "(none)",len(payload["workItems"]),payload["inspectionHash"]));return 0
