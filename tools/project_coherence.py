...
def derive_authorities(sensors):
    ...
    for sensor_name in sorted(sensors):
        ...
        grouped.setdefault(key, {"authority": authority, "observedBy": []})["observedBy"].append(sensor_name)
    out = []
    for key in sorted(grouped):
        group = grouped[key]
        observed_by = sorted(group["observedBy"])
        authority = group["authority"]
        preferred = next((AUTHORITY_IDS[n] for n in observed_by if n in AUTHORITY_IDS), observed_by[0])
        out.append({"id": preferred, "kind": authority.get("kind"), "locator": {n: authority[n] for n in sorted(authority) if n != "kind"}, "observedBy": observed_by})
...
def coherence_check(check_id, status, code, subjects, *, required=True, detail=None):
    try:
        ...
        raise RuntimeError("PROJECT_COHERENCE_STATUS_INVALID") from exc
...
def aggregate_coherence(checks):
    failed = sorted(i["id"] for i in checks if i.get("required") is True and i.get("status") == ObservationStatus.FAIL.value)
    unknown = sorted(i["id"] for i in checks if i.get("required") is True and i.get("status") == ObservationStatus.UNKNOWN.value)
    ...
    return {"status": status, "ok": status != ObservationStatus.FAIL.value, "complete": status == ObservationStatus.PASS.value, "failedChecks": failed, "unknownChecks": unknown, "checks": checks}

def _branch_semantic_domain(head):
    if not isinstance(head, str):
        return None
    try:
        identity = parse_branch_name(head)
    except RuntimeError:
        return None
    if identity.get("grammar") == "canonical" and identity.get("declaredClass") not in {"work", "experiment"}:
        return None
...
def _pull_request_items(sensors):
    ...
    return available, [i for i in items if isinstance(i, dict)]
...
def classify_open_pr(project, pr, bindings):
    ...
    head = pr.get("headRef")
    if isinstance(number, int) and any(binding.get("prNumber") == number for binding in bindings):
        return "work-bound"
    protected = project.get("protectedBranches") or []
    if isinstance(head, str) and head in set(protected):
        return "protected"
    if _branch_semantic_domain(head) == "operations":
        return "operations"
    return "unclassified"
...
def _pr_classification_check(project, sensors, scope):
    ...
    if not available:
        return coherence_check("pull-requests.classification", "UNKNOWN", "REMOTE_PR_INVENTORY_UNAVAILABLE", ["github-pull-requests"], required=required)
    work_available, bindings = _work_bindings(sensors)
    items = [{"number": i.get("number"), "headRef": i.get("headRef"), "classification": classify_open_pr(project, i, bindings)} for i in prs]
    unclassified = [i for i in items if i["classification"] == "unclassified"]
    if unclassified and not work_available:
        return coherence_check("pull-requests.classification", "UNKNOWN", "WORK_AUTHORITY_UNAVAILABLE_FOR_PR_CLASSIFICATION", ["continuations", "github-pull-requests"], required=required, detail={"items": items, "unclassified": unclassified})
    if unclassified:
        return coherence_check("pull-requests.classification", "FAIL", "UNCLASSIFIED_OPEN_PR", ["continuations", "github-pull-requests"], required=required, detail={"items": items, "unclassified": unclassified})
...
def _pr_ci_observation_check(sensors):
    try:
        projection = project_ci_observation.build(sensors.get("pullRequests"))
    except RuntimeError as exc:
        return coherence_check("pull-requests.ci-observation", "FAIL", "PR_CI_EVIDENCE_INVALID", ["github-pull-requests"], required=False, detail={"error": getattr(exc, "code", str(exc).split(":", 1)[0])})
    state = projection["state"]
    if state in {"NOT_APPLICABLE", "GREEN"}:
        status = "PASS"
        code = "NO_OPEN_PRS" if state == "NOT_APPLICABLE" else "PR_CI_GREEN"
    elif state == "FAILED":
        status, code = "FAIL", "PR_CI_FAILED"
    elif state == "PENDING":
        status, code = "UNKNOWN", "PR_CI_PENDING"
    elif state == "REENTRY_REQUIRED":
        status, code = "UNKNOWN", "PR_CI_REENTRY_REQUIRED"
    else:
        status, code = "UNKNOWN", "PR_CI_OBSERVATION_INCOMPLETE"
...
def _lease_pr_check(sensors, scope):
    ...
    by_number = {i.get("number"): i for i in prs if isinstance(i.get("number"), int)}
    missing, mismatch = [], []
    for lease, owner in linked:
        prn = owner["pr"]
        pr = by_number.get(prn)
        if pr is None:
            missing.append({"leaseId": lease.get("leaseId"), "prNumber": prn})
            continue
        branch = owner.get("branch")
        if isinstance(branch, str) and branch != pr.get("headRef"):
            mismatch.append({"leaseId": lease.get("leaseId"), "prNumber": prn, "expected": branch, "observed": pr.get("headRef")})
    if missing:
...
def _work_pr_checks(project, sensors, scope):
    ...
    by_number = {i.get("number"): i for i in prs if isinstance(i.get("number"), int)}
    missing, head_mismatch, base_mismatch = [], [], []
    control = project.get("controlBranch")
    for item in linked:
        prn = item["prNumber"]
        pr = by_number.get(prn)
        if pr is None:
            missing.append({"workId": item.get("workId"), "prNumber": prn})
            continue
        branch = item.get("branch")
        if isinstance(branch, str) and branch != pr.get("headRef"):
            head_mismatch.append({"workId": item.get("workId"), "prNumber": prn, "expected": branch, "observed": pr.get("headRef")})
        if isinstance(control, str) and pr.get("baseRef") != control:
            base_mismatch.append({"workId": item.get("workId"), "prNumber": prn, "expected": control, "observed": pr.get("baseRef")})
...
def evaluate_coherence(project, sensors, *, scope):
    ...
    checks = [_pr_classification_check(project, sensors, scope), _pr_ci_observation_check(sensors), _lease_pr_check(sensors, scope), *_work_pr_checks(project, sensors, scope)]
