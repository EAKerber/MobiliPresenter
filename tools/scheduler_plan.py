...
ACTIONS={"CONTINUE","RECONCILE","HANDOFF","PAUSE","NEEDS_HUMAN"}; ERROR_EXIT=2
...
def continuation_focus(focus):
    return focus.split(":",1)[1] if isinstance(focus,str) and focus.startswith("continuation:") and len(focus.split(":",1)[1])>0 else None

def route(value):
    rec=value["recommendation"];action=rec["action"];focus=rec.get("focus");cid=continuation_focus(focus)
    if action=="HANDOFF":
    ...
    if action=="RECONCILE":return {"shouldWake":True,"channelClass":"supervisor","target":"gitops-supervisor","continuationId":cid}
    if action=="PAUSE":return {"shouldWake":False,"channelClass":"none","target":None,"continuationId":cid}
    if action=="NEEDS_HUMAN":return {"shouldWake":True,"channelClass":"human","target":"human","continuationId":cid}
    raise RuntimeError("SCHEDULER_ACTION_INVALID")

def build_plan(value):
    validate_inspection(value);rec=value["recommendation"]
    body={"schemaVersion":"SchedulerPlan 0.1","inspectionHash":value["inspectionHash"],"action":rec["action"],"reasonCode":str(rec.get("reasonCode") or "UNKNOWN"),"focus":rec.get("focus"),"dispatch":route(value),"decisionScope":"operational-only","semanticAuthority":False,"transportSideEffects":False,"readOnly":True}
...
def main(argv=None):
    ...
    try:
        value=live_inspection() if args.live else load_json(args.input);payload=build_plan(value);print(json.dumps(payload,indent=2 if args.as_json else None,ensure_ascii=False));return 0
