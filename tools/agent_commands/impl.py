...
def run_process(args):
    ...
    return (True, proc.stdout.strip()) if proc.returncode == 0 else (
        False, (proc.stderr or proc.stdout).strip()
    )
...
def observed_git():
    ...
    return {
        "available": True,
        "worktree": True,
        "branch": branch if branch_ok and branch else ci_branch_name(),
...
def git_context_check(state, observed, *, published_source_branch=None):
    ...
    if branch == control or (
        published_source_branch is not None and branch == published_source_branch
    ):
        return {
            "name": "git-context", "status": "PASS", "code": None,
            "context": "control", "branch": branch,
        }
    if branch in protected:
        return {
            "name": "git-context", "status": "PASS", "code": None,
            "context": "protected-parallel", "branch": branch,
        }
    if _operations_branch(branch):
        return {
            "name": "git-context", "status": "PASS", "code": None,
            "context": "operations", "branch": branch,
        }
    return {
        "name": "git-context", "status": "FAIL",
        "code": "UNEXPECTED_BRANCH", "observed": branch,
    }
...
def verify_state():
    ...
    checks.append({
        ...
        "status": "PASS" if SCHEMA_PATH.is_file() else "FAIL",
    ...
    published_source = (
        publication_projection.get("sourceBranch")
...
def project_summary(view):
    return {
        ...
        "controlBranch": view["git"]["controlBranch"],
        "initiative": view["development"]["initiative"],
        "phase": view["development"]["phase"],
        "checkpoint": view["development"]["checkpoint"],
...
def _runtime_inspection(runtime_providers=None):
...
def command_doctor(as_json, runtime_providers=None):
    checks = [
        {"name": "python", "status": "PASS" if sys.version_info >= (3, 10) else "FAIL"},
        {"name": "git-executable", "status": "PASS" if shutil.which("git") else "FAIL"},
        {"name": "gh-executable", "status": "PASS" if shutil.which("gh") else "INFO"},
    ]
    try:
        state = project_state.load_state()
        checks.append({
            "name": "project-state",
            "status": "PASS" if not project_state.validate_current(state) else "FAIL",
        })
    except RuntimeError as exc:
        checks.append({
            "name": "project-state",
    ...
    if git.get("worktree"):
        checks.append({
            ...
            "status": (
                "PASS"
                if "eakerber/mobilipresenter" in str(git.get("origin") or "").lower()
                else "FAIL"
            ),
        })
    else:
        checks.append({"name": "git-worktree", "status": "FAIL"})
    runtime = _runtime_inspection(runtime_providers)
    ok = all(c["status"] in {"PASS", "INFO"} for c in checks)
    payload = {"ok": ok, "checks": checks, "runtimeCapabilities": runtime}
    if as_json:
        print(json.dumps(payload, indent=2, ensure_ascii=False))
    else:
...
def _machine_for_begin(machine_scope, observations_path=None):
    if machine_scope == "local":
        if observations_path:
            raise RuntimeError("AGENT_BEGIN_OBSERVATIONS_REQUIRE_LIVE")
        return project_machine.inspect_local()
    if machine_scope == "base":
        if observations_path:
            raise RuntimeError("AGENT_BEGIN_OBSERVATIONS_REQUIRE_LIVE")
        return project_machine.inspect_base()
    if machine_scope != "live":
        raise RuntimeError("AGENT_BEGIN_MACHINE_SCOPE_INVALID")
    observations = (
        runtime_observations.load_bundle(observations_path)
        if observations_path
        else None
    )
...
def command_begin(as_json, args):
    role = _required(args.role, "AGENT_BEGIN_ROLE_REQUIRED")
    declared_intent = _required(args.intent, "AGENT_BEGIN_INTENT_REQUIRED")
    profile = agent_cycle.entry_profile(role, declared_intent)
    machine = _machine_for_begin(args.machine_scope, args.observations)
    runtime = _runtime_inspection(args.runtime_providers)
    payload = agent_cycle.build_context(
        role=role,
    ...
    if as_json:
        ...
        projection = payload["semanticBrief"]["capabilityProjection"]
        print(
            "AGENT CYCLE BEGIN\n"
            f"  cycle: {payload['cycleId']}\n"
            f"  status: {payload['status']}\n"
            f"  machine-scope: {payload['projectMachine']['scope']}\n"
            f"  required-unavailable: {len(projection['requiredUnavailable'])}\n"
            f"  close: {payload['closeRequirements']['reminder']}"
        )
...
def command_verify(as_json):
    ...
    print(
        ...
        if as_json
        else "\n".join(f"{c['status']:7} {c['name']}" for c in payload["checks"])
    )
...
def command_checkpoint(as_json, args):
    ...
    print(
        json.dumps(payload, indent=2, ensure_ascii=False)
        if as_json
        else ("APPLIED" if args.apply else "PLAN")
    )
    return 0
...
def recent_commits(control_branch):
    ok, out = run_git(
        "log", "--oneline", "--decorate=no", f"{control_branch}..HEAD", "-n", "20"
    )
    return {"available": True, "entries": out.splitlines()} if ok else {
        "available": False, "reason": out
    }
...
def command_git_prune_plan(as_json):
...
def command_git_mutation_plan(as_json, args):
    ...
    if errors:
        raise RuntimeError(f"STATE_SCHEMA_INVALID:{errors[0]['detail']}")
    control = project_state.operational_view(state)["git"]["controlBranch"]
    operation = _required(args.operation, "GIT_MUTATION_OPERATION_REQUIRED")
    if operation == "create-branch":
        plan = git_mutation_plan.create_branch(
            branch=_required(args.branch, "GIT_MUTATION_BRANCH_REQUIRED"),
            base_sha=_required(args.base_sha, "GIT_MUTATION_BASE_SHA_REQUIRED"),
            control_branch=control,
        )
    elif operation == "create-file":
        plan = git_mutation_plan.create_file(
            branch=_required(args.branch, "GIT_MUTATION_BRANCH_REQUIRED"),
            path=_required(args.path, "GIT_MUTATION_PATH_REQUIRED"),
        ...
        plan = git_mutation_plan.create_pr(
            ...
            base=_required(args.base, "GIT_MUTATION_PR_BASE_REQUIRED"),
            head_sha=_required(args.head_sha, "GIT_MUTATION_PR_HEAD_SHA_REQUIRED"),
            title=_required(args.title, "GIT_MUTATION_PR_TITLE_REQUIRED"),
        ...
        plan = git_mutation_plan.merge_pr(
            pr_number=_required(args.pr_number, "GIT_MUTATION_PR_NUMBER_REQUIRED"),
            head_sha=_required(args.head_sha, "GIT_MUTATION_PR_HEAD_SHA_REQUIRED"),
            base=_required(args.base, "GIT_MUTATION_PR_BASE_REQUIRED"),
            control_branch=control,
        ...
        plan = git_mutation_plan.update_ref(
            branch=_required(args.branch, "GIT_MUTATION_BRANCH_REQUIRED"),
            current_sha=_required(args.current_sha, "GIT_MUTATION_CURRENT_REF_SHA_REQUIRED"),
            new_sha=_required(args.new_sha, "GIT_MUTATION_NEW_REF_SHA_REQUIRED"),
            control_branch=control,
            force=args.force,
        )
    else:
    ...
    print(
        ...
        if as_json
            ...
            f"  connector: {plan['connectorAction']}\n"
...
def main():
    parser = argparse.ArgumentParser(
        prog="agent", description="MobiliPresenter deterministic operational toolbox"
    )
    parser.add_argument(
        "command",
    ...
    parser.add_argument("--machine-scope", choices=("local", "base", "live"), default="local")
    ...
    try:
        ...
        if args.command == "doctor":
            return command_doctor(args.as_json, args.runtime_providers)
        ...
        if args.command == "git":
            if args.apply:
                raise RuntimeError(
                    "UNSUPPORTED_TRANSITION: git planning is read-only; "
                    "apply remains a separately guarded connector/domain operation"
                )
            if args.subcommand == "prune-plan":
                return command_git_prune_plan(args.as_json)
            if args.subcommand == "mutation-plan":
                return command_git_mutation_plan(args.as_json, args)
