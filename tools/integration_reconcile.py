...
ERROR_EXIT=2;SCHEMA_VERSION="IntegrationReconcilePlan 0.2"
...
def boundary_assessment(head_ref,changed_files):
    ...
    if domain=="engine":
        ...
        for path in files:
            if path.startswith(("viewer-next/src/presentation/","viewer-next/src/runtime/","viewer-next/src/renderer/","viewer-next/src/fixtures/","scene-core/")):violations.append({"path":path,"code":"UI_TOUCHED_ENGINE_DOMAIN"})
            elif path.startswith("viewer-next/src/api/"):reviews.append({"path":path,"code":"SHARED_API_CONTRACT_REVIEW"})
...
def main():
    ...
    try:
        state=load_state();view=project_state.operational_view(state);repository=view["project"]["repository"];target=args.target or view["git"]["controlBranch"];plan=build_plan(GhObserver(repository).observe(args.pr,target));print(json.dumps(plan,indent=2,ensure_ascii=False) if args.as_json else render_text(plan));return 0
