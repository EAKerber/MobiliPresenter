...
SHARED = (
    "viewer-next/src/api/", "viewer-next/src/bootstrap.ts", "viewer-next/index.html",
    "viewer-next/package.json", "viewer-next/tsconfig.json", ".github/workflows/",
)
...
def classify_path(path: str) -> str:
    prefixes = (
        ("viewer-next/src/api/", "viewer-api"), ("viewer-next/src/ui/", "viewer-ui"),
        ("viewer-next/src/presentation/", "viewer-presentation"), ("viewer-next/src/runtime/", "viewer-runtime"),
        ("viewer-next/src/renderer/", "viewer-renderer"), ("viewer-next/tests/", "viewer-tests"),
        ("scene-core/", "scene-core"), (".github/workflows/", "ci"), ("docs/", "docs"),
    )
...
def is_shared_resource(path: str) -> bool:
    return any(path.startswith(x) if x.endswith("/") else path == x for x in SHARED)


def boundary_assessment(head_ref: str, changed_files: Iterable[str]) -> dict[str, Any]:
    files, violations, reviews = sorted(set(changed_files)), [], []
    ...
    return {
        "sharedResourcesTouched": [p for p in files if is_shared_resource(p)],
        "boundaryReview": reviews, "boundaryViolations": violations,
    }


def aggregate_ci(runs: list[dict[str, Any]], head_sha: str | None, head_ref: str = "") -> dict[str, Any]:
    latest: dict[str, dict[str, Any]] = {}
    for run in runs:
        name = str(run.get("name") or "")
        if not name or (name == "Agent Ops" and not head_ref.startswith("ops/")): continue
    ...
    if not selected: status = "unknown"
        ...
        conclusions = {str(r.get("conclusion") or "").lower() for r in selected}
        status = "green" if conclusions <= {"success", "neutral", "skipped"} else (
            "failed" if conclusions & {"failure", "cancelled", "timed_out", "action_required", "startup_failure"} else "unknown"
        )
    return {"status": status, "validatedSha": head_sha, "runs": [
        {k: r.get(k) for k in ("name", "id", "status", "conclusion")} for r in selected
    ]}


def domain_summary(files: Iterable[str]) -> dict[str, int]:
    out: dict[str, int] = {}
...
def state_assessment(state: dict[str, Any], pr: dict[str, Any]) -> dict[str, Any]:
    git_state, dev = state.get("git", {}), state.get("development", {})
    active, active_pr = git_state.get("activeDevelopmentBranch"), dev.get("prNumber")
    alignment = "aligned-active-development" if (active, active_pr) == (pr.get("headRef"), pr.get("number")) else (
        "no-active-development" if active is None and active_pr is None else "development-identity-mismatch"
    )
    stale = []
    ...
    return {
        "alignment": alignment, "activeDevelopmentBranch": active, "activePrNumber": active_pr,
        "phase": dev.get("phase"), "checkpoint": dev.get("checkpoint"), "nextTransition": dev.get("nextTransition"),
        "blockers": dev.get("blockers") or [], "likelyStaleFields": stale,
        "stateReviewRecommended": bool(pr.get("merged")),
        "postMergeReviewFields": ["git.activeDevelopmentBranch", "development.prNumber", "development.phase", "development.checkpoint", "development.nextTransition", "development.blockers"],
    }


def recommendation(obs: dict[str, Any], boundary: dict[str, Any], ci: dict[str, Any]) -> dict[str, Any]:
    pr, target, ancestry = obs["pr"], obs["target"], obs["ancestry"]
    base_to_target, target_to_head = ancestry["declaredBaseToTarget"], ancestry["targetToHead"]
    if pr.get("merged"): action, reason = "already-merged", "pull-request-is-already-merged"
        ...
        action, reason = (("retarget-to-control-and-revalidate", "declared-base-is-contained-in-control") if base_to_target.get("status") in {"ahead", "identical"} else ("manual-reconciliation", "declared-base-is-not-cleanly-contained-in-control"))
...
def build_plan(obs: dict[str, Any]) -> dict[str, Any]:
    pr, files = obs["pr"], sorted(set(obs.get("changedFiles") or []))
    boundary = boundary_assessment(str(pr.get("headRef") or ""), files)
    ci = aggregate_ci(obs.get("workflowRuns") or [], pr.get("headSha"), str(pr.get("headRef") or ""))
    body = {
        "schemaVersion": SCHEMA_VERSION, "repository": obs["repository"], "pr": pr, "target": obs["target"], "ancestry": obs["ancestry"],
        "scope": {"changedFileCount": len(files), "changedFiles": files, "domains": domain_summary(files), **boundary},
        "ci": ci, "canonicalState": state_assessment(obs["projectState"], pr),
        "recommendation": recommendation(obs, boundary, ci), "applyEligible": False,
...
class GhObserver:
    ...
    def _pages(self, endpoint: str) -> list[Any]:
        ...
        while True:
            ...
            page += 1
    @staticmethod
    def _compare(value: Any) -> dict[str, Any]:
        if not isinstance(value, dict): return {"status": "unknown", "aheadBy": None, "behindBy": None, "mergeBaseSha": None}
        merge_base = value.get("merge_base_commit") if isinstance(value.get("merge_base_commit"), dict) else {}
        return {"status": value.get("status"), "aheadBy": value.get("ahead_by"), "behindBy": value.get("behind_by"), "mergeBaseSha": merge_base.get("sha")}
    def observe(self, pr_number: int, target_branch: str, project_state: dict[str, Any]) -> dict[str, Any]:
        ...
        target_sha, base_sha, head_sha = target_commit.get("sha"), base.get("sha"), head.get("sha")
        ...
        return {
            "repository": repo,
            "pr": {"number": pr.get("number"), "state": pr.get("state"), "draft": pr.get("draft"), "merged": bool(pr.get("merged")), "mergeable": pr.get("mergeable"), "headRef": head.get("ref"), "headSha": head_sha, "baseRef": base.get("ref"), "baseSha": base_sha},
            "target": {"branch": target_branch, "sha": target_sha},
            "ancestry": {"declaredBaseToTarget": self._compare(self._run(f"repos/{repo}/compare/{base_sha}...{target_sha}")), "targetToHead": self._compare(self._run(f"repos/{repo}/compare/{target_sha}...{head_sha}"))},
            "changedFiles": [x.get("filename") for x in self._pages(f"repos/{repo}/pulls/{pr_number}/files") if isinstance(x, dict) and isinstance(x.get("filename"), str)],
            "workflowRuns": workflows.get("workflow_runs", []) if isinstance(workflows, dict) else [], "projectState": project_state,
        }
...
def render_text(plan: dict[str, Any]) -> str:
    pr, target, a, scope, rec = plan["pr"], plan["target"], plan["ancestry"], plan["scope"], plan["recommendation"]
...
def main() -> int:
    ...
    parser.add_argument("command", choices=("reconcile-plan",)); parser.add_argument("pr", type=int); parser.add_argument("--target"); parser.add_argument("--json", action="store_true", dest="as_json")
    ...
    try:
        ...
        plan = build_plan(GhObserver(repository).observe(args.pr, target, state))
        print(json.dumps(plan, indent=2, ensure_ascii=False) if args.as_json else render_text(plan)); return 0
