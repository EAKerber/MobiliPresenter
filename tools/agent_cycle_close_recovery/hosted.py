...
def _canonical_close_runner(args: list[str]) -> tuple[int, dict[str, Any]]:
    ...
    if parsed.command != "close":
        raise hosted.HostedAgentCycleError("HOSTED_AGENT_CLOSE_ACTION_REQUIRED")
    payload = agent_cycle_close.close_from_files(
        context_path=parsed.context,
        machine_scope=parsed.machine_scope,
        observations_path=parsed.observations,
        runtime_providers=parsed.runtime_providers,
        evidence_paths=parsed.evidence,
    )
    rc = 0 if payload["status"] == "PASS" else 2 if payload["status"] == "BLOCKED" else 1
    return rc, payload


def close_with_compatibility_recovery(
    command: dict[str, Any],
    meta: dict[str, int],
    *,
    begin_dir: str,
    ...
    hosted._run_agent = _canonical_close_runner
    try:
        try:
            return hosted.close_from_envelope(
                outer,
                meta,
                begin_dir=begin_dir,
        ...
        hosted._run_agent = original_runner
    ...
    try:
        ...
        hosted.validate_begin_manifest(manifest, context)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("close", nargs="?")
    ...
    try:
        ...
        result = close_with_compatibility_recovery(
            command_value,
            meta,
