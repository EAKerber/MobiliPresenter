...
SCHEMA_VERSION = "AgentCycleTouchedResourceSet 0.1"
...
def _exact_locator(kind: str, value: Any) -> dict[str, Any]:
    ...
    expected: dict[str, set[str]] = {
        ...
        "pull-request-slot": {"repository", "head", "base"},
    ...
    if kind in {"git-branch", "git-path", "pull-request-slot", "pull-request", "lease-scope"}:
    ...
    elif kind == "pull-request-slot":
        _text(out.get("head"), "AGENT_CYCLE_RESOURCE_PR_SLOT_INVALID")
        _text(out.get("base"), "AGENT_CYCLE_RESOURCE_PR_SLOT_INVALID")
    elif kind == "pull-request":
        _positive_int(out.get("number"), "AGENT_CYCLE_RESOURCE_PR_INVALID")
...
def origin(source_kind: str, source_hash: str, operation: str | None = None) -> dict[str, Any]:
    value = {
        ...
        "operation": None if operation is None else _text(operation, "AGENT_CYCLE_RESOURCE_ORIGIN_INVALID"),
    }
    return value
...
def validate_origin(value: Any) -> dict[str, Any]:
    ...
    expected = origin(value.get("sourceKind"), value.get("sourceHash"), value.get("operation"))
    if value != expected:
        raise RuntimeError("AGENT_CYCLE_RESOURCE_ORIGIN_INVALID")
    return value


def resource(kind: str, locator: dict[str, Any], *origins: dict[str, Any]) -> dict[str, Any]:
    ...
    canonical_origins.sort(key=lambda item: (item["sourceKind"], item["sourceHash"], item["operation"] or ""))
    if not canonical_origins or len(canonical_origins) != len({stable_hash(item) for item in canonical_origins}):
...
def _merge_resources(items: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for raw in items:
        ...
        current["origins"] = sorted(
            ...
            key=lambda entry: (entry["sourceKind"], entry["sourceHash"], entry["operation"] or ""),
        )
...
def build_resource_set(
    *, repository: str, cycle_instance_id: str, resources: Iterable[dict[str, Any]]
    ...
    canonical = _merge_resources(resources)
    origin_hashes = {
        stable_hash(entry)
        for item in canonical
        for entry in item["origins"]
    }
    source_kinds = sorted({
        entry["sourceKind"]
        for item in canonical
    ...
    body = {
        ...
        "resources": canonical,
        "sourceSummary": {
            "resourceCount": len(canonical),
            "originCount": len(origin_hashes),
...
def _git_branch(branch: str, source: dict[str, Any]) -> dict[str, Any]:
    return resource("git-branch", {"repository": REPOSITORY, "branch": branch}, source)
...
def resources_from_git_plan(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if operation == "create-pr":
        return [resource(
            "pull-request-slot",
            {"repository": REPOSITORY, "head": target["head"], "base": target["base"]},
            src,
        )]
    if operation == "merge-pr":
        found = [resource(
            "pull-request",
            {"repository": REPOSITORY, "number": target["prNumber"]},
            src,
        )]
        base = plan["preconditions"].get("expectedBase")
        if isinstance(base, str) and base:
            found.append(_git_branch(base, src))
        return found
...
def resources_from_transition_plan(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    src = origin("transition-plan", plan["planHash"], f"{plan['domain']}.{plan['action']}")
...
def resources_from_agent_tool_dispatch(dispatch: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    src = origin(
        ...
        dispatch["command"]["target"].get("operation") or dispatch["command"]["target"].get("action"),
    )
    nested = resources_from_remote_command(dispatch["command"])
    return [resource(item["kind"], item["locator"], src) for item in nested]


def resources_from_write_lease_request(request: dict[str, Any]) -> list[dict[str, Any]]:
...
def resources_from_write_lease_result(result: dict[str, Any]) -> list[dict[str, Any]]:
    from tools import agent_write_lifecycle
