"""Git-backed canonical writer for live WorkItems / ContinuationState 0.2."""
...
class GitHubContinuationAuthority:
    ...
    @property
    def update_endpoint(self)->str: return f"repos/{self.repository}/git/refs/heads/{quote(self.authority_branch,safe='')}"

    def _json(self,response,operation:str)->Any:
        try:return json.loads(response.body)
        except json.JSONDecodeError as exc:raise ContinuationRemoteError("CONTINUATION_REMOTE_INVALID_RESPONSE",operation) from exc
    ...
    def observe(self)->Observation:
        ...
        try:transition.validate_work_inventory(items)
        except RuntimeError as exc:raise ContinuationRemoteError("WORK_AUTHORITY_INVALID",str(exc)) from exc
        return Observation(head,tree,items)

    def _commit_item(self,observed:Observation,cid:str,candidate:dict[str,Any],action:str)->str:
        content=json.dumps(candidate,indent=2,ensure_ascii=False)+"\n"
        ...
        tree=self._sha(self._json(self.transport.request("POST",f"repos/{self.repository}/git/trees",payload={"base_tree":observed.tree_sha,"tree":[{"path":f"{self.state_dir}/{cid}.json","mode":"100644","type":"blob","sha":blob}]}),"create tree").get("sha"),"tree")
        commit=self._sha(self._json(self.transport.request("POST",f"repos/{self.repository}/git/commits",payload={"message":f"work: {action} {cid}","tree":tree,"parents":[observed.head_sha]}),"create commit").get("sha"),"commit")
    ...
    def apply(self,planned:dict[str,Any],expected_plan:str|None)->dict[str,Any]:
        try:
            transition.validate_plan(planned,repository=self.repository,authority_branch=self.authority_branch,state_dir=self.state_dir)
            protocol.require_expected_plan(planned,expected_plan)
            continuation.require_current(planned["candidate"],planned["subject"]["id"])
        except RuntimeError as exc:raise ContinuationRemoteError(str(exc).split(":",1)[0]) from exc
        cid=planned["subject"]["id"]; observed=self.observe(); current=observed.items.get(cid)
        if continuation.state_hash(current)!=planned["beforeStateHash"]:raise ContinuationRemoteError("CONTINUATION_PLAN_STALE")
        inventory=[copy.deepcopy(value) for _,value in sorted(observed.items.items())]
        try:
            transition.validate_plan(planned,current,repository=self.repository,authority_branch=self.authority_branch,state_dir=self.state_dir,bind_before=True,inventory=inventory)
            candidate_items=copy.deepcopy(observed.items); candidate_items[cid]=copy.deepcopy(planned["candidate"]); transition.validate_work_inventory(candidate_items)
        except RuntimeError as exc:
            code=str(exc).split(":",1)[0]
            if code=="CONTINUATION_PLAN_STALE":raise ContinuationRemoteError(code) from exc
            raise ContinuationRemoteError("CONTINUATION_PLAN_INVALID",code) from exc
        self._commit_item(observed,cid,planned["candidate"],planned["action"])
        last=None
        for attempt in range(self.readback_attempts):
            readback=self.observe(); last=readback; value=readback.items.get(cid)
            if continuation.state_hash(value)==planned["afterStateHash"]:
                try:
                    transition.validate_work_inventory(readback.items); receipt=protocol.build_receipt(planned,value,authority_revision=readback.head_sha); protocol.validate_receipt(receipt,planned); return receipt
                except RuntimeError as exc:raise ContinuationRemoteError("CONTINUATION_RECEIPT_INVALID",str(exc)) from exc
