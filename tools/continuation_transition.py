...
def create(cid: str, actor: str, remaining: list[str], next_action: str, branch: str | None = None, pr: int | None = None) -> dict[str, Any]:
    ...
    after = {
        "schemaVersion": continuation.SCHEMA,
        "id": cid,
        "actor": actor,
        "status": "READY",
        "branch": branch,
...
def advance(before: dict[str, Any], done: list[str], next_action: str | None = None, sha: str | None = None, checkpoint: str | None = None) -> dict[str, Any]:
    after = copy.deepcopy(continuation.valid(before))
    if after["status"] not in {"READY", "IN_PROGRESS"}:
...
def wait(before: dict[str, Any], blocked: list[str]) -> dict[str, Any]:
    ...
    return _build("wait", before, after, {"blockedBy": blocked})
...
def resume(before: dict[str, Any], actor: str) -> dict[str, Any]:
    after = copy.deepcopy(continuation.valid(before))
    actor = continuation.text(actor, "CONTINUATION_ACTOR_INVALID")
    if after["status"] not in {"WAITING", "HANDOFF"}:
        raise RuntimeError("CONTINUATION_RESUME_STATUS_INVALID")
    if after["status"] == "HANDOFF" and after["handoffTo"] != actor:
        raise RuntimeError("CONTINUATION_HANDOFF_ACTOR_MISMATCH")
    after["actor"] = actor
    after["status"] = "IN_PROGRESS"
    after["blockedBy"] = []
    after["handoffTo"] = None
    return _build("resume", before, after, {"actor": actor})


def done(before: dict[str, Any]) -> dict[str, Any]:
    after = copy.deepcopy(continuation.valid(before))
    if after["status"] not in {"READY", "IN_PROGRESS"}:
...
def rebuild(plan: dict[str, Any], before: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if action == "wait":
        return wait(before, intent.get("blockedBy"))
    if action == "handoff":
        return handoff(before, intent.get("to"), intent.get("nextAction"))
    if action == "resume":
        return resume(before, intent.get("actor"))
    if action == "done":
        return done(before)
