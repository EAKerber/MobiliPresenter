...
from tools import project_machine, project_sensors, routines
...
def cap(policy="canonical", action="NO_EXPERIMENTAL_REVIEW", *, isolated=False):
    return {
        "id": "cap",
        "policy": policy,
        "supervisorParticipation": "isolated" if isolated else "active",
        "reviewAction": action,
        "nextGates": ["g1"] if action == "TEST_NEXT_GATES" else [],
        "backlogCount": 1 if action == "TEST_NEXT_GATES" else 0,
        "roundsWithoutActiveGates": 3 if action == "REVIEW_EMPTY_LIMIT" else 0,
...
def work(
    ...
    *,
    ...
    return {
        ...
        "stateHash": (work_id[0] if work_id else "a") * 64,
    }
...
def sensors(work_items=None, prs=None, capabilities=None):
    ...
    return {
        "projectState": project_sensors.sensor(
            "PASS",
            data={"verification": verification, "checks": []},
            authority={"kind": "repository", "path": "ops/state/project.json"},
        ),
        "publication": project_sensors.sensor(
            "PASS",
            data={"checks": []},
            authority={"kind": "repository", "path": "ops/published/current.json"},
        ),
        ...
        "repository": project_sensors.sensor(
            "PASS",
            data={"checks": []},
            authority={"kind": "repository", "name": "EAKerber/MobiliPresenter"},
        ),
        "control": project_sensors.sensor(
            "PASS",
            data={"branch": "main", "sha": "2" * 40, "mode": "remote"},
            authority={"kind": "git-ref", "branch": "main"},
        ),
        "capabilities": project_sensors.sensor(
            "PASS",
            data={"items": capabilities if capabilities is not None else [cap()]},
            authority={"kind": "repository", "path": "ops/capabilities"},
        ),
        "pullRequests": project_sensors.sensor(
            "PASS",
            data={"available": True, "items": prs or []},
...
def machine(work_items=None, prs=None, capabilities=None, *, scope="live"):
    return project_machine.build_inspection(
        state(), sensors(work_items, prs, capabilities), scope=scope
    )


def derive(m):
    routine = routines.build_inspection(m)
    return routine, maintenance.from_inputs(m, routine)


class MaintenanceInspectTests(unittest.TestCase):
    def test_canonical_input_is_project_machine_plus_routine_inspection(self):
        m = machine()
        routine, value = derive(m)
        self.assertEqual(value["schemaVersion"], "MaintenanceInspection 0.6")
        self.assertEqual(value["projectMachineInspectionHash"], m["inspectionHash"])
        self.assertEqual(value["routineInspectionHash"], routine["inspectionHash"])
        self.assertTrue(maintenance.validate_derivation(value, m, routine)["ok"])
        self.assertEqual(maintenance.from_project_inspection(m), value)

    def test_no_work_continues_project_transition(self):
        _, value = derive(machine())
    ...
    def test_handoff_precedes_runnable_work_and_carries_target_decision(self):
        _, value = derive(machine([work("a", "HANDOFF", target="developer-engine"), work("b")]))
    ...
    def test_runnable_work_carries_worker_target(self):
        _, value = derive(machine([work("a", worker="developer-engine")]))
        rec = value["recommendation"]
    ...
    def test_pending_ci_does_not_pause_independent_runnable_work(self):
        ...
        _, value = derive(machine(items, prs))
        self.assertEqual(value["recommendation"]["action"], "CONTINUE")
        self.assertEqual(value["recommendation"]["workId"], "b")
    ...
    def test_single_work_ci_states_are_policy_not_sensor_trust(self):
        ...
        _, pending = derive(machine([item], [{**base, "ci": "pending"}]))
        ...
        _, failed = derive(machine([item], [{**base, "ci": "failed"}]))
        self.assertEqual(failed["recommendation"]["action"], "RECONCILE")
        _, unknown = derive(machine([item], [{**base, "ci": "unknown", "ciObserved": False}]))
    ...
    def test_waiting_and_dependency_blocked_work_pause_only_without_independent_progress(self):
        _, waiting = derive(machine([work("a", "WAITING")]))
        self.assertEqual(waiting["recommendation"]["action"], "PAUSE")
        _, dep = derive(machine([work("a", depends=["dep"]), work("dep", "WAITING")]))
        self.assertEqual(dep["recommendation"]["action"], "PAUSE")
        _, independent = derive(machine([work("a", "WAITING"), work("b")]))
        self.assertEqual(independent["recommendation"]["action"], "CONTINUE")
        self.assertEqual(independent["recommendation"]["workId"], "b")

    def test_active_experimental_capability_is_supplied_by_routine(self):
        experimental = cap("experimental", "TEST_NEXT_GATES")
        routine, value = derive(machine([work("a", "WAITING")], capabilities=[experimental]))
        self.assertEqual(routine["results"][0]["findings"][0]["code"], "CAPABILITY_GATES_DUE")
        self.assertEqual(value["recommendation"]["action"], "CONTINUE")
        self.assertEqual(value["recommendation"]["focus"], "cap")

    def test_capability_empty_limit_preserves_legacy_operational_semantics(self):
        experimental = cap("experimental", "REVIEW_EMPTY_LIMIT")
        _, value = derive(machine(capabilities=[experimental]))
        self.assertEqual(value["recommendation"]["action"], "NEEDS_HUMAN")
        self.assertEqual(value["recommendation"]["reasonCode"], "CAPABILITY_EMPTY_LIMIT")

    def test_isolated_experimental_capability_is_monitored_but_not_actionable(self):
        experimental = cap("experimental", "TEST_NEXT_GATES", isolated=True)
        routine, value = derive(machine(capabilities=[experimental]))
        self.assertFalse(routine["results"][0]["findings"][0]["supervisorEligible"])
        self.assertEqual(value["recommendation"]["reasonCode"], "NEXT_TRANSITION_AVAILABLE")
    ...
    def test_routine_unknown_and_fail_are_explicit_findings(self):
        for status, code, expected in (
            ("UNKNOWN", "CAPABILITY_OBSERVATION_UNKNOWN", "ROUTINE_INSPECTION_INCOMPLETE"),
            ("FAIL", "CAPABILITY_OBSERVATION_FAILED", "ROUTINE_INSPECTION_FAILED"),
        ):
            s = sensors()
            s["capabilities"] = project_sensors.sensor(
                status,
                code=code,
                data={"items": []},
                authority={"kind": "repository", "path": "ops/capabilities"},
            )
            m = project_machine.build_inspection(state(), s, scope="live")
            routine = routines.build_inspection(m)
            value = maintenance.from_inputs(m, routine)
            self.assertEqual(routine["status"], status)
            self.assertIn(expected, [item["code"] for item in value["findings"]])

    def test_routine_lineage_tampering_is_rejected_even_if_rehashed(self):
        m = machine()
        routine = routines.build_inspection(m)
        tampered = copy.deepcopy(routine)
        tampered["results"][0]["applicable"] = True
        body = {k: v for k, v in tampered.items() if k != "inspectionHash"}
        tampered["inspectionHash"] = routines.stable_hash(body)
        with self.assertRaisesRegex(RuntimeError, "ROUTINE_DERIVATION_MISMATCH"):
            maintenance.from_inputs(m, tampered)

    def test_hash_and_derivation_reject_maintenance_tampering(self):
        m = machine([work("a")])
        routine, value = derive(m)
        ...
        with self.assertRaisesRegex(RuntimeError, "MAINTENANCE_DERIVATION_MISMATCH"):
            maintenance.validate_derivation(rehashed, m, routine)

    def test_maintenance_no_longer_has_direct_capability_deathcircle_path(self):
        self.assertFalse(hasattr(maintenance, "_capability_findings"))
