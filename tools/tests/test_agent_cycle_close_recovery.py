...
class AgentCycleCloseMergeRecoveryTests(unittest.TestCase):
    ...
    def test_hosted_workflow_routes_compatibility_to_recovery_wrapper(self):
        workflow = (Path(__file__).resolve().parents[2] / ".github" / "workflows" / "hosted-agent-cycle.yml").read_text(encoding="utf-8")
        marker = "- name: Run canonical close compatibility recovery"
        compatibility = workflow.split(marker, 1)[1]
        self.assertIn("python tools/hosted_agent_cycle_close_recovery.py close", compatibility)
        initial = workflow.split(marker, 1)[0]
