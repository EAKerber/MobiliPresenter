...
class OperationalSemanticsCoverageTests(unittest.TestCase):
    def test_live_inventory_has_complete_deterministic_coverage(self):
        ...
        self.assertGreater(first["catalogCounts"]["entrypoints"], 0)
        self.assertGreater(first["catalogCounts"]["workflows"], 0)
    ...
    def test_missing_entrypoint_binding_is_explicit(self):
        ...
        registry["toolSurfaces"]["python-module-cli"]["bindings"] = [item for item in bindings if item["target"] != "semantic-cli"]
        inspection = coverage.build_inspection(registry)
        self.assertIn("tools/semantics/__main__.py", inspection["findings"]["missingEntrypointBindings"])
    ...
    def test_unregistered_schema_is_explicit(self):
        ...
        inspection = coverage.build_inspection(registry)
        self.assertIn("ops/schemas/ecosystem-maxims.schema.json", inspection["findings"]["unregisteredSchemas"])
        self.assertFalse(inspection["coverageComplete"])

    def test_tampered_inspection_is_rejected(self):
        inspection = coverage.build_inspection()
        inspection["coverageComplete"] = False
        with self.assertRaisesRegex(RuntimeError, "OPERATIONAL_SEMANTICS_COVERAGE_MISMATCH"):
