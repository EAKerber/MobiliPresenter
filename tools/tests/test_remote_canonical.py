...
class RemoteCanonicalTests(unittest.TestCase):
    def test_request_hash_is_deterministic_and_read_only(self):
        ...
        self.assertIs(left["semanticAuthority"], False)
        self.assertIs(left["authorizesMutation"], False)

    def test_tampered_request_is_rejected(self):
        request = _coordination_request()
        request["declaredIntent"]["goal"] = "changed-after-hash"
        with self.assertRaisesRegex(RuntimeError, "REMOTE_REQUEST_HASH_MISMATCH"):
            remote_canonical.validate_request(request)

    def test_coordination_bridge_is_semantically_identical_to_direct_planner(self):
        request = _coordination_request()
    ...
    def test_continuation_bridge_is_semantically_identical_to_direct_planner(self):
    ...
    def test_unknown_route_fails_closed(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "REMOTE_DOMAIN_PLANNER_UNAVAILABLE"):
            remote_canonical.plan_request(request, [_coordination_observation()])

    def test_planner_identity_mismatch_fails_closed(self):
        request = _coordination_request(
            planner={"id": "tools.continuation_transition", "contract": "TransitionPlan 0.1"}
        )
        with self.assertRaisesRegex(RuntimeError, "REMOTE_PLANNER_MISMATCH"):
            remote_canonical.plan_request(request, [_coordination_observation()])

    def test_authority_drift_fails_before_planning(self):
        request = _coordination_request()
        with self.assertRaisesRegex(RuntimeError, "REMOTE_AUTHORITY_DRIFT"):
            remote_canonical.plan_request(request, [_coordination_observation("b" * 40)])

    def test_observation_set_must_be_closed(self):
        request = _coordination_request()
        extra = {
            "authority": remote_canonical.CONTINUATION_AUTHORITY,
            "revision": HEAD,
        ...
        with self.assertRaisesRegex(RuntimeError, "REMOTE_OBSERVATION_SET_MISMATCH"):
            remote_canonical.plan_request(request, [_coordination_observation(), extra])

    def test_scope_cannot_silently_widen_beyond_expected_authorities(self):
        with self.assertRaisesRegex(RuntimeError, "REMOTE_SCOPE_MUST_MATCH_EXPECTED_AUTHORITIES"):
    ...
    def test_rp1a_has_no_executor_or_remote_transport_dependency(self):
        source = inspect.getsource(remote_canonical)
        self.assertNotIn("coordination_remote", source)
        self.assertNotIn("continuation_remote", source)
        self.assertNotIn("project_state_apply", source)
