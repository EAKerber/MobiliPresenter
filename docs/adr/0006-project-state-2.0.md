...
`git.preserveBranches` became `git.protectedBranches`. The new name is intentionally narrow: it means an explicit current guard against automatic branch sanitation. It does **not** mean retained, archived, active, canonical, authoritative or permanently preserved. M6 owns branch lifecycle/retention.
...
Every value embedded in `ProjectState 1.0 development.constraints` required an explicit destination before removal. M4A materialized `ops/migrations/project-state-2.0.json`, which classified each current constraint as one of:
...
The live migration required exact coverage of the V1 constraint list with no duplicate or unresolved entry. The migration map was temporary migration evidence, not a replacement global constraints document.

## M4A / M4B boundary

### M4A — consumer decoupling

M4A did **not** mutate `ops/state/project.json` and did **not** replace the canonical `ops/schemas/project-state.schema.json`.
...
During M4A, `validate_current()` accepted only ProjectState 1.0.

### M4B — authority migration

M4B performed the actual state/schema transition using the existing ProjectState canonical writer and Transition Protocol semantics: exact before-state, deterministic candidate, expected plan identity, atomic apply, readback and rollback.

M4B completed the authority migration with a verified `TransitionReceipt 0.1`, removed temporary V1 runtime compatibility, and left the canonical ProjectState schema accepting ProjectState 2.0 only. The migration map remains recoverable at source control revision `031ce02039030078fdca3b40b282f1f789edca09`; durable transition evidence is stored at `ops/evidence/project-state/project-state-2.0-migration.json`.

## Consequences

Positive:

- ProjectState bootstrap is substantially smaller;
...
- consumers are insulated from the raw schema through a small operational projection;
- M5 and M6 can migrate work and branch lifecycle independently.

Costs:

- M4A required a temporary V1/V2 compatibility layer and migration evidence;
- ProjectMachineInspection changed from 0.2 to 0.3 because its public project projection renamed `preserveBranches` to `protectedBranches`;
- Handoff changed from 1.0 to 2.0 because it no longer embeds the raw ProjectState.
